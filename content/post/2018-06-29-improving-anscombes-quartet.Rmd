...
As much as I like it, it has a **teaching flaw**, 
so let's try to improve upon this great data set.
As intented by its creator Francis Anscombe, the quartet shows that you never, never
(did I forget to 
say "never"?) should
_blindly_ use statistical estimates. Instead you should check your implicit assumptions by making 
charts of your data.
Anscombes quartet furthermore makes clear that outliers can heavily bias your statistical result.

R is shipped with the `anscombe` data set:
...
I often use the quartet in data visualisation courses. I start 
...
Before introducing the didactical flaw (just a little bit more patience...), 
...
The Anscombe data set contains four different data sets each describing the
abstract variables $x$  and $y$ having abritary values. $x$ and $y$ may be 
...
Lets tweak the data set so that it becomes more "alive". Lets turn the data set from Anscombe quartet into
__Anscombes quarters__.

## Anscombe's quarters[^quarters] (fiction)

The city councel of Anscombe ordered a small survey to monitor the socio-economic differences between
its districts. Although a wealthy city, the councel had concerns that 
there were large differences in (average) income as well as (average) age. It would also be 
interesting to know if there was a relation between age and income. An old class-mate of one of the council members living in the Eastern quarter conducted the survey had returned with 
a quick result.

[^quarters]: Accidentally in some languages quartet is semantically connected to a synonym 
...
So the council as assured that there were absolutely no differences, but should they?

On his birthday party the council member was shocked to find out that many of his old class-mates,
were interviewed for the survey by their mutual friend.  His friend advised to plot the data: 

```{r anscombe_districts}
ggplot(anscombe_d, aes(x = age, y = income)) +  
  geom_point() + 
  geom_smooth(method="lm", se = T, col="red", alpha=0.1) +
  facet_wrap(~ district) +  scale_y_continuous(labels = function(x){paste0(x, "k$")})
```
